 ...
 * ModelStorage supports (e.g. FitEvent,UnsupervisedEvent, TransfromEvent), we store entries in the Event table.
 */
public class EventDao {
     ...
     */
    public static EventRecord store(int eventId, String eventType, int experimentRunId, int stageNum, DSLContext ctx) {
        EventRecord ev = ctx.newRecord(EVENT);
        ev.setId(null);
        ev.setExperimentrun(experimentRunId);
        ev.setEventtype(eventType);
        ev.setEventid(eventId);
        ev.setStagenumber(stageNum);
        ev.store();
        return ev;
    }
