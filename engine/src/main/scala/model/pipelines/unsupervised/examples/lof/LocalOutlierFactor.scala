...
private[ml] trait LOFParams extends Params with HasFeaturesCol with HasOutputCol {
  final val minPts: Param[Int] = new IntParam(this, "minPts", "a minimum number of points")
  setDefault(minPts -> 5)
  final def getMinPts: Int = $(minPts)

  final val distType: Param[String] = new Param[String](this, "distType", "the type of distance")
  setDefault(distType -> "euclidean")
  final def getDistType: String = $(distType)
}

class LocalOutlierFactor(override val uid: String) extends Transformer with LOFParams {
  ...
  override def transformSchema(schema: StructType): StructType = {
    ...
    StructType(Array(
      StructField(LocalOutlierFactor.index,
        DataTypes.LongType,
        nullable = false),
      StructField(LocalOutlierFactor.lof,
        DataTypes.DoubleType,
        nullable = false),
      StructField(LocalOutlierFactor.vector,
        VectorType,
        nullable = false)
    ))
  }

  // TODO: The time complexity of this function is O(n * k).
  /**
  ...
  def computeKDistanceNeighborhood(
    ...
    data.foreach { case (idx: Long, vec: Vector) =>
      val targetDist = distType match {
        case LocalOutlierFactor.euclidean => math.sqrt(Vectors.sqdist(target, vec))
   ...
   * @return
   */
  def combineNeighborhood(first: Array[(Long, Double)], second: Array[(Long, Double)])
  : Array[(Long, Double)] = {
    var pos1 = 0
    var pos2 = 0
    var count = 0 // the size of distinct instances
