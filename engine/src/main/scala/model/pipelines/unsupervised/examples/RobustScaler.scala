...
import org.apache.spark.ml.util.SchemaUtils.appendColumn
...
class RobustScaler (override val uid: String)
  ...
  override def fit(dataset: Dataset[_]): RobustScalerModel = {
    ...
    val numFeatures = getNumFeatures(dataset, $(inputCol))
    ...
    if (numFeatures <= 1000) {
      vectors.mapPartitions { iter =>
        if (iter.hasNext) {
          ...
          while (iter.hasNext) {
            val vec = iter.next
            vec.foreachActive { (i, v) => if (!v.isNaN) summaries(i) = summaries(i).insert(v) }
          }
...
class RobustScalerModel private[ml] (override val uid: String,
                                     val range: Vector,
                                     val median: Vector)
  ...
  def getTransformFunc(shift: Array[Double],
                       scale: Array[Double],
                       withShift: Boolean,
  ...
  override def transformSchema(schema: StructType): StructType = {
    ...
    if ($(outputCol).nonEmpty) {
      outputSchema = updateAttributeGroupSize(outputSchema,
