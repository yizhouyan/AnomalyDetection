...
class SVM (override val uid: String)
  ...
  private def train_(dataset: Iterator[LabeledPoint]): SVMModel = {
    ...
    if($(kernelType) == "linear"){
      ...
      new SVMLinearModel(uid, Vectors.dense(w).asML, b)
    }
    else{
  ...
  override protected def train(dataset: Dataset[_]): SVMModel = {
    ...
    val subModels = lpData.mapPartitions{iter => Iterator(train_(iter))}.cache()
    if ($(kernelType) == "linear") {
      val tmp_w = subModels.map(_.asInstanceOf[SVMLinearModel].weight)
      val tmpw = tmp_w.map(i => Vectors.fromML(i))
      val w = Statistics.colStats(tmpw).mean
      val b = subModels.map(_.asInstanceOf[SVMLinearModel].b).mean()
      new SVMLinearModel(uid, w.asML, b)
    }
    else {
